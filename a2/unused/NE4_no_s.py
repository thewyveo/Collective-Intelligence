...
class PredatorPreyConfig(Config):
    ...
    predator_energy_consumption: float = 0.5
    predator_lunge_energy_consumption: float = 1.0
    ...
    def evolve(self):
        for gen in tqdm(range(self.config.generations), desc="Evolving"):
            ...
            for idx, (prey, pred) in enumerate(zip(self.prey_population, self.predator_population)):
                ...
                print(f"Prey {(idx+1)}/{len(self.prey_population)} fitness: {prey_fitnesses[-1]:.4f}")
                args = (deepcopy(pred), deepcopy(self.prey_test_population), self.config)
                predator_fitnesses.append(evaluate_predator_task(args))
                print(f"Predator {(idx+1)}/{len(self.predator_population)} fitness: {predator_fitnesses[-1]:.4f}")
...
class NeuralPrey(Agent):
    ...
    def get_nn_state(self):
        ...
        return torch.tensor([
            ...
            0.0  # was on-site flag, now always 0
        ], dtype=torch.float32)
    ...
    def update(self):
        ...
        # STATE CONTROL: Blend FSM and NN state decisions
        if self.brain is not None and hasattr(self, "last_nn_output") and self.last_nn_output is not None:
            ...
            # Get NN state preference
            nn_state_probs = F.softmax(state_logits, dim=0)
            nn_preferred_state = PREY_WANDERING if nn_state_probs[0] > nn_state_probs[1] else PREY_FLEEING
            
            # Get FSM state decision
            fsm_state = self._get_fsm_state()
            
            # Blend state decisions based on confidence
            if self.confidence > 0.5:  # NN has more control over state
                if random.random() < self.confidence:
                    self.state = nn_preferred_state
                else:
                    self.state = fsm_state
            else:  # FSM has more control over state
                if random.random() < (1.0 - self.confidence):
                    self.state = fsm_state
                else:
                    self.state = nn_preferred_state
        else:
            # Pure FSM state control
    ...
    def change_position(self):
        ...
        # Blend FSM and NN behavior based on confidence
        if self.brain is not None and hasattr(self, "last_nn_output") and self.last_nn_output is not None:
            ...
            if move_vec.length() > 0:
                ...
                self.move = fsm_move.lerp(nn_move, self.confidence)
            else:
...
class NeuralPredator(Agent):
    ...
    def update(self):
        ...
        if self.brain is not None and hasattr(self, "last_nn_output") and self.last_nn_output is not None:
            ...
            # Get NN state preference
            nn_state_probs = F.softmax(state_logits, dim=0)
            nn_preferred_state = PREDATOR_HUNTING if nn_state_probs[0] > nn_state_probs[1] else PREDATOR_LUNGING
            
            # Get FSM state decision
            fsm_state = self._get_fsm_state(nearest_prey, min_distance)
        ...
        # Handle target assignment for lunging
        if self.state == PREDATOR_LUNGING and nearest_prey:
            self.target_prey = nearest_prey
        elif self.state == PREDATOR_HUNTING:
            self.target_prey = None
        ...
        if self.state == PREDATOR_LUNGING and nearest_prey and min_distance < self.config.predator_eating_threshold:
            # Caught prey!
...
def plot_final_simulation(final_data):
    ...
    axes[1, 0].plot(final_data['frame'], pred_hunting, label='Hunting', color='orange')
    axes[1, 0].plot(final_data['frame'], pred_lunging, label='Lunging', color='darkred')
