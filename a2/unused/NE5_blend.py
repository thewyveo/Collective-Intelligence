...
CONSCIOUSNESS_THRESHOLD = 0.9  # Threshold for consciousness
    ...
    for idx, predator in enumerate(predator_list):
        for _ in range(config.n_simulations):
            ...
            if len(df) < 10:
                ...
                positions = [a.pos for a in sim.agents if isinstance(a, NeuralPrey) and a.alive]
                if len(positions) > 1:
                    dispersion = sum(
                        (p1 - p2).length() for i, p1 in enumerate(positions) for j, p2 in enumerate(positions) if i < j
                    ) / (len(positions) * (len(positions) - 1) / 2)
                    dispersion_score = min(dispersion / sim.config.window.as_tuple()[0], 1.0)  # Normalize to [0, 1]
                else:
...
class NeuralPrey(Agent):
    ...
    def on_spawn(self):
        ...
        self.steps_since_decision = random.randint(0, self.decision_interval)
    ...
    def get_nn_state(self):
        ...
        return torch.tensor([
            ...
            pred_count / 20.0 if pred_count != 0 else 0.0,             # 5: number of predators nearby (capped at 10)
            ...
            prey_count / 20.0 if prey_count != 0 else 0.0,  # 10: number of other prey nearby (capped at 20)
    ...
    def update(self):
        ...
        # Improved state control with smoother transitions
        if self.brain is not None and hasattr(self, "last_nn_output") and self.last_nn_output is not None:
            ...
            # Blend between FSM and NN states based on confidence
            if self.confidence > 0:  # Even low confidence can use some NN influence
                nn_state_probs = F.softmax(state_logits, dim=0)
                
                # Calculate blended state probabilities
                fsm_state = self._get_fsm_state()
                fsm_probs = torch.tensor([1.0 if fsm_state == PREY_WANDERING else 0.0, 
                                        1.0 if fsm_state == PREY_FLEEING else 0.0])
                
                blended_probs = self.confidence * nn_state_probs + (1 - self.confidence) * fsm_probs
                
                # State transition with hysteresis to prevent rapid flipping
                if blended_probs[0] > blended_probs[1] * 1.2:  # Requires 20% stronger signal to switch
                    new_state = PREY_CONSCIOUS_W if self.confidence > CONSCIOUSNESS_THRESHOLD else PREY_WANDERING
                else:
                    new_state = PREY_CONSCIOUS_F if self.confidence > CONSCIOUSNESS_THRESHOLD else PREY_FLEEING
                    
                # Smooth transition
                if new_state != self.state:
                    if random.random() < 0.7:  # 70% chance to follow new state
    ...
    def change_position(self):
        ...
        fsm_move = self._get_fsm_movement()
        
        if self.brain is not None and hasattr(self, "last_nn_output") and self.last_nn_output is not None:
            _, move_x, move_y, speed_ctrl = self.last_nn_output
            nn_move = Vector2(move_x.item(), move_y.item())

            noise_strength = 0.05
            nn_move += Vector2(random.uniform(-noise_strength, noise_strength),
                   random.uniform(-noise_strength, noise_strength))
            
            if nn_move.length() > 0:
                nn_move = nn_move.normalize()
                
                # Blend FSM and NN movements based on confidence
                blend_factor = min(self.confidence * 1.5, 1.0)  # Scale confidence for more aggressive NN control
                blended_move = nn_move * blend_factor + fsm_move * (1 - blend_factor)
                
                # Set speed based on state and confidence
                if self.state in [PREY_FLEEING, PREY_CONSCIOUS_F]:
                    base_speed = self.config.prey_flee_speed
                else:
                    base_speed = self.config.prey_speed
                    
                # Apply speed control from NN, but keep minimum speed
                speed = max(base_speed * (0.5 + speed_ctrl.item()/2), base_speed * 0.7)
                
                self.move = blended_move.normalize() * speed
            else:
                self.move = fsm_move
        else:
            self.move = fsm_move
        ...
        if self.on_site():
            if random.random() < 0.5:
                self.move *= 0.75  # Consistent slowing on site
            else:
                self.move *= 1.15
    ...
    def _get_fsm_movement(self):
        ...
        if self.state in [PREY_WANDERING, PREY_CONSCIOUS_W]:
            # More purposeful wandering when conscious
            if self.confidence > CONSCIOUSNESS_THRESHOLD and random.random() < 0.7:
                # Head toward food or away from predators more deliberately
                nearest_food = None
                ...
                for agent, dist in self.in_proximity_accuracy():
                    if isinstance(agent, Food) and dist < min_food_dist:
                        nearest_food, min_food_dist = agent, dist
                    elif isinstance(agent, NeuralPredator) and dist < min_pred_dist:
                        nearest_pred, min_pred_dist = agent, dist
                
                if nearest_food and min_food_dist < self.config.prey_vision * 0.7:
                    return (nearest_food.pos - self.pos).normalize() * self.config.prey_speed
                elif nearest_pred and min_pred_dist < self.config.prey_vision * 0.5:
                    return (self.pos - nearest_pred.pos).normalize() * self.config.prey_speed
    ...
    def update_confidence(self):
        ...
        age_factor = min(self.age / 300.0, 1.0)
        energy_factor = min(self.energy / (self.config.prey_energy if isinstance(self, NeuralPrey) 
                                        else self.config.predator_energy), 1.0)
        
        # Successful actions boost confidence faster
        success_factor = 1.0
        if isinstance(self, NeuralPrey) and self.energy > self.config.prey_energy * 0.8:
            success_factor *= 1.2
        if isinstance(self, NeuralPredator) and hasattr(self, 'target_prey') and self.target_prey:
            success_factor *= 1.1
            
        self.confidence = min(age_factor * energy_factor * success_factor, 1.0)
...
class NeuralPredator(Agent):
    ...
    def get_nn_state(self):
        ...
        return torch.tensor([
            ...
            prey_dir.x, prey_dir.y,
            prey_count / 20.0 if prey_count != 0 else 0.0,  # 4: number of prey nearby (capped at 20)
            ...
            pred_count / 20.0 if pred_count != 0 else 0.0,  # 6: number of predators nearby (capped at 20)
            pred_dir.x, pred_dir.y,
            pred_distance / self.config.prey_vision,
            0.0 if self.sex == "m" else 1.0,
    ...
    def change_position(self):
        ...
        if self.target_prey and self.target_prey.alive:
            target_vec = (self.target_prey.pos - self.pos)
        ...
        if self.brain is not None and hasattr(self, "last_nn_output") and self.last_nn_output is not None:
            ...
            if nn_move.length() > 0:
                ...
                # Blend NN movement with target direction based on state
                if self.state in [PREDATOR_LUNGING, PREDATOR_CONSCIOUS_L]:
                    # Strong target focus when lunging
                    move_blend = nn_move * 0.3 + target_dir * 0.7
                else:
                    # More exploratory when hunting
                    move_blend = nn_move * 0.7 + target_dir * 0.3
                    
                move_blend = move_blend.normalize()
                
                # Speed control
                if self.state in [PREDATOR_LUNGING, PREDATOR_CONSCIOUS_L]:
                    base_speed = self.config.predator_lunge_speed
                else:
                    base_speed = self.config.predator_speed
                    
                speed = base_speed * (0.8 + 0.4 * speed_ctrl.item())  # 80-120% of base speed
                
                self.move = move_blend * speed
            else:
