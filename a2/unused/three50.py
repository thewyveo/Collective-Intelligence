...
class RockPaperScissorsConfig(Config):
    ...
    initial_species_a = initial_species_b = initial_species_c = 100
    
    # Parameters for species interactions
    species_a_birth_rate = species_b_birth_rate = species_c_birth_rate = 0.01
    species_a_death_rate = species_b_death_rate = species_c_death_rate = 0.005
    a_kills_c_rate = b_kills_a_rate = c_kills_b_rate = 0.0006
    species_a_reproduction_rate = species_b_reproduction_rate = species_c_reproduction_rate = 0.3
...
class SpeciesC(Agent):  # Scissors - kills B, killed by A
    ...
    def update(self):
        ...
        if species_b_agents and random.random() < self.config.a_kills_c_rate:
...
def run_multiple_simulations(n_runs=50):
    ...
    for i in range(n_runs):
        ...
        # Store best duration
        if df["frame"].iloc[-1] > best_duration:
            best_duration = df["frame"].iloc[-1]
            best_df = df.copy()

        # Plot best simulation
        plt.figure(figsize=(12, 6))
        plt.plot(best_df["frame"], best_df["species_a"], label="Species A (Rock)", color="green", linewidth=2)
        plt.plot(best_df["frame"], best_df["species_b"], label="Species B (Paper)", color="blue", linewidth=2)
        plt.plot(best_df["frame"], best_df["species_c"], label="Species C (Scissors)", color="red", linewidth=2)
        plt.title("Best Simulation Run (Longest Duration)")
        plt.xlabel("Frame")
        plt.ylabel("Population")
        plt.grid(True, alpha=0.3)
        plt.legend()
        plt.savefig("50best_three.png", dpi=300)
        plt.close()

        # Plot all 50 simulations
        plt.figure(figsize=(16, 8))
        for df in all_dfs:
            plt.plot(df["frame"], df["species_a"], color="green", alpha=0.3, linewidth=0.7)
            plt.plot(df["frame"], df["species_b"], color="blue", alpha=0.3, linewidth=0.7)
            plt.plot(df["frame"], df["species_c"], color="red", alpha=0.3, linewidth=0.7)

        plt.title("All 50 Simulations - Population Dynamics")
        plt.xlabel("Frame")
        plt.ylabel("Population")
        plt.grid(True, alpha=0.3)
        plt.savefig("all_simulation_runs.png", dpi=300)
        plt.close()
