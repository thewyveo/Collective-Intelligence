...
class RockPaperScissorsConfig(Config):
    ...
    b_kills_c_rate = 0.000087   #0.00087
    c_kills_a_rate = 0.00053 #0.00053

    self_crowd_coeff = 0.00133 #0.00133
    capacity = 3000
    # Energy parameters
    ...
    duration: int = 5000
    movement_speed: float = 10.0 #10.0
...
def batch_run_rps(n_runs=50):
    ...
    # Concatenate and compute average per frame
    full_df = pd.concat(all_runs)

    # Save to CSV
    full_df.to_csv("rps_50.csv", index=False)
    print("Saved: rps_50.csv")

    plt.figure(figsize=(12, 6))
    # Plot average population over time
    for run_id in full_df["run"].unique():
        run_df = full_df[full_df["run"] == run_id]
        plt.plot(run_df["frame"], run_df["species_a"], color="green", alpha=0.3)
        plt.plot(run_df["frame"], run_df["species_b"], color="red", alpha=0.3)
        plt.plot(run_df["frame"], run_df["species_c"], color="blue", alpha=0.3)
    plt.title("Average Population Dynamics (50 RPS Simulations)")
    plt.xlabel("Frame")
    plt.ylabel("Population")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig("rps_50_runs.png", dpi=300, bbox_inches='tight')
    plt.close()
    print("Saved: rps_50_runs.png")

if __name__ == "__main__":
    batch_run_rps(n_runs=50)